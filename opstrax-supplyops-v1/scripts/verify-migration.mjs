...
try {
  ...
  for (const table of ['wms_capacity_units','wms_handling_units','wms_space_occupancies','wms_space_reservations','wms_billable_events','wms_operational_events']) {
    try { selectOne(`SELECT COUNT(*) AS count FROM ${table}`); } catch { throw new Error(`WMS migration missing table ${table}`); }
  }
  process.stdout.write(`[verify-migration] OK All ${EXPECTED_VERSION} migrations verified, including WMS capacity/economics tables.\n`);
  process.exit(0);
} catch (error) {
  process.stderr.write(`[verify-migration] ERROR ${error.message}\n`); process.exit(1);
}
